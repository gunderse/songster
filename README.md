...
- New specials (similar to skip and steal): 1) 50/50 skill that eliminates the possibility of half of the wrong answer slots.  2) a distraction skill that plays an annoying sound on the hub over the song being played to make it harder to hear.  it should play a random audio file from the the assets/effects/annoy/ folder the contents of which can change over time.  deploying this skill means the other team cannot "play again" or "extend play" of the clip either.
- specials should be a resource configurable on the admin screen at game start with an amout per team limit (0 is an option disabling it.)
- add a button to skip intro during intro display.
- don't show the song name for the team's initial milestone year card drawn - just the year
- if the final reveal showcase hasn't finished generating by the time the snippet ends continue playing the snippent until transitioning to the showcase to fill the awkward gap in time.
